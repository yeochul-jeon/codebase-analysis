...
function resolveIndex(db: DbAdapter, repoName: string, commit?: string): DbIndex | undefined {
  const repo = db.getRepoByName(repoName);
  if (!repo) return undefined;

  if (commit) {
    const idx = db.getIndex(repo.id, commit);
    return idx?.status === 'ready' ? idx : undefined;
  }

  const head = db.getRepoHead(repo.id, repo.default_branch);
  if (!head) return undefined;
  const idx = db.getIndexById(head.index_id);
  return idx?.status === 'ready' ? idx : undefined;
}
...
export function createReadsRouter(db: DbAdapter, blob: BlobAdapter): Hono {
  ...
  router.get('/v1/search', (c) => {
    ...
    const idx = resolveIndex(db, repo, commit);
    if (!idx) return c.json({ error: 'Not found' }, 404);

    const symbols = db.searchSymbols(idx.repo_id, idx.commit_sha, q, limit);
    return c.json({ symbols });
  });

  // GET /v1/symbols/:key
  router.get('/v1/symbols/:key', (c) => {
    const key = c.req.param('key');
    if (!HEX64.test(key)) return c.json({ error: 'Invalid symbol key' }, 400);

    const symbol = db.getSymbolByKey(key);
  ...
  router.get('/v1/symbols/:key/body', async (c) => {
    ...
    const idx = db.getIndexById(symbol.index_id);
    if (!idx) return c.json({ error: 'Not found' }, 404);
  ...
  router.get('/v1/repos/:name/files', (c) => {
    const name = c.req.param('name');
    const commit = c.req.query('commit');

    const idx = resolveIndex(db, name, commit);
    if (!idx) return c.json({ error: 'Not found' }, 404);

    const files = db.getFilesByIndex(idx.id);
    return c.json({ files });
  });

  // GET /v1/symbols/:key/references
  router.get('/v1/symbols/:key/references', (c) => {
    const key = c.req.param('key');
    if (!HEX64.test(key)) return c.json({ error: 'Invalid symbol key' }, 400);
    ...
    const occurrences = db.getOccurrences(symbol.name, idx.repo_id, idx.commit_sha);
    return c.json({ symbol_key: key, occurrences });
  });

  // GET /v1/repos/:name/file-symbols?path=&commit=
  router.get('/v1/repos/:name/file-symbols', (c) => {
    const name = c.req.param('name');
    ...
    const symbols = db.getFileSymbols(idx.repo_id, idx.commit_sha, path);
